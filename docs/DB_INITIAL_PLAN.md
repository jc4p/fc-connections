# FC-Connections Dynamic Database Schema for Cloudflare D1
...
## Initial Field Definitions Setup
...
```sql
-- Get complete profile data for a specific user's profile
SELECT p.id AS profile_id, p.profile_type, p.view_count, 
       fd.field_key, fd.field_label, fd.field_type, fv.field_value
FROM profiles p
JOIN profile_field_values fv ON p.id = fv.profile_id
JOIN profile_field_definitions fd ON fv.field_id = fd.id
WHERE p.fid = ? AND p.profile_type = ?
ORDER BY fd.field_order;

-- Search for profiles with specific field values (e.g., finding job profiles in tech industry)
SELECT p.*, u.display_name, u.avatar_url
FROM profiles p
JOIN users u ON p.fid = u.fid
JOIN profile_field_values fv ON p.id = fv.profile_id
JOIN profile_field_definitions fd ON fv.field_id = fd.id
WHERE p.profile_type = 'job' 
AND fd.field_key = 'industry' 
AND fv.field_value LIKE '%tech%'
ORDER BY p.view_count DESC
LIMIT 20;

-- Get most viewed profiles of a specific type
SELECT p.id, p.fid, u.display_name, p.view_count
FROM profiles p
JOIN users u ON p.fid = u.fid
